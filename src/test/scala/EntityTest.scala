...
class EntityTest extends AnyFlatSpec {
  ...
  it should "die" in {
    ...
    val res: Player = testPlayer.die
    assert(res.inventory.items == Vector()) //one example to what happens when player dies
  }
  ...
  it should "tick" in {
    ...
    val test = player.addEffect(effect1).addEffect(effect2)
    println("Tick test: " + test.tick.get.tick.get.tick.get.tick.get.tick.get.tick.get.tick.get.tick)
